...
export function HazardDrawPage() {
  const navigate = useNavigate()

  // Outer context — used for light/3D prefs and post-save refresh
  const { lightPreset, show3D, refreshHazardLayers, toggleHazard } = useMapContext()
  const { selectedCity, cityId, cityBoundary } = useCityContext()

  // Own map engine — does NOT register with outer MapProvider
  ...
  const [state, dispatch] = useReducer(hazardDrawReducer, HAZARD_DRAW_INITIAL)

  // Drawing composable bound to local engine
  ...
  // ── Sync light/3D prefs to local engine ────────────────────────────────────
  ...
  useEffect(() => {
    ...
    // Intentionally fires only when draw.drawnGeometry changes
  ...
  async function handleSave() {
    ...
    try {
      ...
      const params = new URLSearchParams({ hazard_type: state.hazardType, scenario: state.scenario })
      await fetch(`${API_URL}/cities/${cityId}/hazards/regenerate-pmtiles?${params}`, {
        method: 'POST', credentials: 'include',
      })

      await refreshHazardLayers()
      toggleHazard(`${state.hazardType}::${state.scenario}`)
      ...
      <aside className="w-72 shrink-0 flex flex-col border-r bg-background overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b shrink-0">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-sm font-semibold truncate">Draw Hazard Area</span>
